...
AFFTP_PlayerCameraManager::AFFTP_PlayerCameraManager(const FObjectInitializer& ObjectInitializer)
...
void AFFTP_PlayerCameraManager::BeginPlay()
{
	Super::BeginPlay();

}

void AFFTP_PlayerCameraManager::UpdateCamera(float DeltaTime)
{
	AFFTP_Character* MyPawn = PCOwner ? Cast<AFFTP_Character>(PCOwner->GetPawn()) : nullptr;
	if (MyPawn)
	{
